...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable(name="W_x", dtype=tf.float32, shape=(self.input_size, self.state_size),
                                  initializer=tf.contrib.layers.xavier_initializer())
            W_h = tf.get_variable(name="W_h", dtype=tf.float32, shape=(self.state_size, self.state_size),
                                  initializer=tf.contrib.layers.xavier_initializer())
            b = tf.get_variable(name="b", dtype=tf.float32, shape=(self.state_size, ), initializer=tf.contrib.layers.xavier_initializer())

            new_state = tf.nn.sigmoid(tf.add(tf.add(tf.matmul(inputs, W_x), tf.matmul(state, W_h)), b))
            ### END YOUR CODE ###
