...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(len(labels)):
            windowed_data.append(([t for r in sentence[i:i+2*window_size+1] for t in r], labels[i]))
        ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        h_dropout = tf.nn.dropout(h, self.config.dropout)
        pred = tf.add(tf.matmul(h_dropout, U), b2)
        ### END YOUR CODE
        return pred
